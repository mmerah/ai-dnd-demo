...
You must first read the "INVESTIGATION_REPORT.md" file from the investigator agent that is located inside the claude-instance-{id} directory that was automatically created for this claude session. Then use ultrathink to trace execution paths, dependencies, and file interconnections based on the files identified in that report and while you are analyzing each flow path, immediately update the "FLOW_REPORT.md" inside the claude-instance-{id} directory.
