"""Dedicated agent for major NPCs with persistent minds."""
...
class IndividualMindAgent(BaseNPCAgent):
    """Stateful agent that represents a single major NPC."""
    ...
    def _build_context(self, game_state: GameState, npc: NPCInstance) -> str:
        ...
        if dialogue_session.target_npc_ids:
            ...
            if targeted_names:
                sections.append("Other participants in this exchange: " + ", ".join(targeted_names))
    ...
    def _build_prompt(
        ...
        parts.append(
            "Instructions:\n"
            "- Respond as the NPC in first person.\n"
            "- Keep tone consistent with persona and memories.\n"
            "- Offer logical next steps or questions when appropriate."
        )
        parts.append(f"Player message: {player_message}")
        return "\n\n".join(part for part in parts if part)

    def _summarize_recent_dialogue(self, game_state: GameState, npc: NPCInstance, limit: int = 6) -> str:
        history: list[str] = []
