...
async def create_new_game(request: NewGameRequest) -> NewGameResponse:
    ...
    try:
        ...
        if not characters_path.exists():
            raise HTTPException(
                status_code=500, 
                detail="Characters data not found"
            )
        ...
        if not character_data:
            raise HTTPException(
                status_code=404,
                detail=f"Character with ID '{request.character_id}' not found"
            )
    ...
    try:
        ...
        if not narrative:
            ...
            await broadcast_service.publish(
                game_id,
                "error",
                {"error": "AI response was empty, using fallback response"}
            )
        ...
        if updated_game_state:
            ...
            await message_service.send_character_update(
                game_id,
                updated_game_state.character.model_dump()
            )
            ...
            await message_service.send_game_update(
                game_id,
                game_state_dict
            )
