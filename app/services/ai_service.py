...
The current game state and character information will be provided with each interaction."""
    ...
    def _register_tools(self) -> None:
        ...
        logger.info("Registered 16 tools with the agent successfully")
...
- Time: Day {game_state.game_time.day}, {game_state.game_time.hour:02d}:{game_state.game_time.minute:02d}""")
    ...
    def _convert_messages_to_pydantic_ai(self, messages: list[Message]) -> list[ModelMessage]:
        ...
        for msg in messages:
            if msg.role == MessageRole.SYSTEM:
                ...
                pydantic_messages.append(ModelRequest(parts=[SystemPromptPart(content=msg.content)]))
    ...
    async def _extract_and_broadcast_tool_events(self, game_id: str, messages: list[ModelMessage]) -> None:
            ...
            if isinstance(msg, ModelResponse):
                ...
                for part in msg.parts:
                    if isinstance(part, ToolCallPart):
                        ...
                        await message_service.send_tool_call(game_id, part.tool_name, part.args)
                                ...
                                for next_part in next_msg.parts:
                                    if isinstance(next_part, ToolReturnPart) and next_part.tool_name == part.tool_name:
                                        ...
                                        await message_service.send_tool_result(
                                            game_id, part.tool_name, next_part.content
                                        )

    async def _handle_stream_events(
        self, ctx: RunContext[AgentDependencies], event_stream: AsyncIterator[AgentStreamEvent], game_id: str
    ) -> None:
        ...
        async for event in event_stream:
            if isinstance(event, FunctionToolCallEvent):
                ...
                await message_service.send_tool_call(game_id, event.part.tool_name, event.part.args)
                ...
                await message_service.send_tool_result(game_id, event.tool_call_id, event.result.content)
            elif isinstance(event, PartDeltaEvent):
                # Handle text streaming - but we'll use stream_text() for actual text
                if isinstance(event.part_delta, TextPartDelta):
                    logger.debug(
                        f"Text delta received: {len(event.part_delta.content_delta) if event.part_delta.content_delta else 0} chars"
                    )
    ...
    async def generate_response(
        self, user_message: str, game_state: GameState, game_service: GameService, stream: bool = True
        ...
        """
        deps = AgentDependencies(game_state=game_state, game_service=game_service, dice_service=DiceService())
            ...
            if stream:
                ...
                try:
                    ...
                    async def event_handler(
                        ctx: RunContext[AgentDependencies], events: AsyncIterator[AgentStreamEvent]
                    ):
                    ...
                    async with self.agent.run_stream(
                        ...
                        async for text_chunk in run_stream.stream_text(delta=True):
                            ...
                            yield {"type": "narrative_chunk", "content": text_chunk}
                ...
                run_result = await self.agent.run(full_user_message, deps=deps, message_history=message_history)
                ...
                yield {"type": "complete", "narrative": narrative}
