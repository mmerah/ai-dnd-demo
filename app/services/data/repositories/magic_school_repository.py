...
class MagicSchoolRepository(BaseRepository[MagicSchool]):
    def __init__(self, path_resolver: IPathResolver, cache_enabled: bool = True):
        ...
        self.file = self.path_resolver.get_shared_data_file("magic_schools")
    ...
    def _load_all(self) -> None:
        ...
        if not data or not isinstance(data, dict):
            return
        for item in data.get("magic_schools", []):
            try:
    ...
    def _load_item(self, key: str) -> MagicSchool | None:
        data = self._load_json_file(self.file)
        if not data or not isinstance(data, dict):
            return None
        for item in data.get("magic_schools", []):
    ...
    def _get_all_keys(self) -> list[str]:
        ...
        if not data or not isinstance(data, dict):
            return []
        return sorted([i.get("index", "") for i in data.get("magic_schools", []) if i.get("index")])
