...
async def update_hp(
    ...
    logger.info(f"HP Update: {target} {'healed' if amount > 0 else 'took'} {abs(amount)} {damage_type} - HP: {old_hp} → {new_hp}/{max_hp}")
    return result
...
async def add_condition(
    ...
    logger.info(f"Condition Added: {target} is now {condition} {'for ' + str(duration) + ' rounds' if duration > 0 else 'until removed'}")
    return result
...
async def update_spell_slots(
    ...
    if not character.spellcasting:
        return {
            ...
            "level": level,
            "error": "Character has no spellcasting ability"
        }
    ...
    if level_key not in spell_slots:
        return {
            "type": "spell_slots_update",
            "level": level,
            "error": f"No spell slots for level {level}"
        }
