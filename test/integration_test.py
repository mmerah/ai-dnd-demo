...
class IntegrationTestRunner:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                for char in characters:
                    ...
                    assert all(
                        key in char for key in ["id", "name", "race", "class_name", "level", "hit_points", "abilities"]
                    )
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/api/game/new", json={"character_id": character_id, "scenario_id": scenario_id}
                )
    ...
    async def test_get_game_state(self, game_id: str) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                assert all(
                    key in game_state for key in ["character", "location", "game_time", "npcs", "conversation_history"]
                )

                print("✓ Game state retrieved:")
                print(f"  - Location: {game_state['location']}")
                print(
                    f"  - Character: {game_state['character']['name']} (HP: {game_state['character']['hit_points']['current']}/{game_state['character']['hit_points']['maximum']})"
                )
                print(
    ...
    async def test_player_action_with_sse(self, game_id: str, action: str) -> None:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(f"{self.base_url}/api/game/{game_id}/action", json={"message": action})
            ...
            if not narrative_chunks and last_dm_message:
                ...
                print("  ✗ No narrative content received or saved")
                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("event:"):
                            ...
                            if event_type and event_data:
                                try:
                                    data = json.loads(event_data)
                                    event_record = {"event": event_type, "data": data, "timestamp": time.time()}
    ...
    async def test_ai_narrative_streaming(self, game_id: str, skip_ai: bool = False) -> None:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    f"{self.base_url}/api/game/{game_id}/action", json={"message": test_action}
                )
                assert response.status_code == 200, f"Failed to send action: {response.status_code}"
            ...
            if narrative_chunks:
                ...
                print("  ✗ No narrative chunks received!")
                ...
                if last_dm_msg and last_dm_msg["content"]:
                    ...
                    print("  ✗ No content generated at all!")
                    self.test_results["ai_streaming"] = False
    ...
    async def test_error_handling(self, game_id: str) -> None:
        ...
        test_cases = [
            {"name": "Invalid game ID", "test": self._test_invalid_game_id, "expect_error": True},
            {"name": "Invalid character ID", "test": self._test_invalid_character_id, "expect_error": True},
            {
                "name": "Empty message",
    ...
    async def _test_empty_message(self, game_id: str) -> None:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{self.base_url}/api/game/{game_id}/action", json={"message": ""})
    ...
    async def run_all_tests(self) -> None:
        ...
        try:
            ...
            game_id = await self.test_create_game(character_id, scenario_id if scenario_id else "")
